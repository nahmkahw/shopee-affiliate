...
async function loadChars(){const j=await fetch('/api/maprang/characters').then(r=>r.json());const el=document.getElementById('char-list');const chars=j.characters||{};if(!Object.keys(chars).length){el.innerHTML='<span style="color:#475569;font-size:12px">ยังไม่มีตัวละคร</span>';return;}el.innerHTML=Object.values(chars).map(c=>'<div style="background:#0f172a;border:1px solid #1e293b;border-radius:8px;padding:8px;font-size:12px;min-width:120px">'+(c.ref_image?'<img src="/dashboard/maprang/charimg/'+c.id+'?t='+Date.now()+'" style="width:100%;aspect-ratio:2/3;object-fit:cover;border-radius:4px;margin-bottom:4px">':'<div style="aspect-ratio:2/3;background:#1e293b;border-radius:4px;margin-bottom:4px;display:flex;align-items:center;justify-content:center;color:#475569;font-size:11px;text-align:center">ยังไม่มีรูป</div>')+'<div style="font-weight:600">'+(c.name||c.id)+'</div><div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:4px"><button class="btn btn-purple btn-sm" data-id="'+c.id+'" onclick="genCharImg(this.dataset.id)" title="สร้างรูปด้วย AI">🎨</button><button class="btn btn-ghost btn-sm" data-id="'+c.id+'" onclick="uploadCharImg(this.dataset.id)" title="อัปโหลดรูปเอง">⬆</button><button class="btn btn-ghost btn-sm" data-id="'+c.id+'" onclick="editChar(this.dataset.id)">แก้</button><button class="btn btn-red btn-sm" data-id="'+c.id+'" onclick="delChar(this.dataset.id)">ลบ</button></div></div>').join('');}
