...
## แสดงผล queue (grilling รอบ 2)

แสดง **holder + waiters** บน main hub dashboard:
- **waiters tracking:** ไฟล์ต่อ pid `comfyui-gpu.waiter.<pid>.json` (เลี่ยง shared-write race) — `acquire` register ตอนเริ่มรอ, ลบตอนได้ lock/exit
- `readQueueStatus()` → `{ holder:{agent,since}, waiters:[{agent,since}] }` + prune PID ที่ตาย
- endpoint `GET /api/gpu-queue` ([common.js](../agent-hub/routes/common.js)) → การ์ด "🎮 ComfyUI GPU" บน hub ([main.js](../agent-hub/html/main.js)) poll ทุก 4s แสดง "รัน: maprang (3 นาที) · รอ: anime, mammuang" / "● ว่าง"
- ✅ verify: endpoint คืน holder จริง, dead waiter ถูก prune, การ์ด+poll อยู่ในหน้าที่เสิร์ฟ
