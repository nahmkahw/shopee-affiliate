...
- **บทพูดจาก Concept สรุป** (grilling รอบ 6): comic เป็น "explainer" — input ดิบ (ข่าว/บทความ/ข้อมูล) → `summarizeConcept()` สรุปเป็น `{title, points[3-4]}` (`meta.concept`) → บทพูดสร้างจาก concept. map: `panelBrief` P1=hook(title) · P2=point0 · P3=point1 · P4=takeaway(points ที่เหลือ) → ส่งใจความเข้า per-panel prompt ให้บทสนทนาสื่อ point นั้น. `deriveSharedSetting` รับ `concept.title` → เลือก "ที่ตัวละครนั่งคุยเรื่องนี้" (default ร้านกาแฟถ้านามธรรม). dashboard comic mode เปลี่ยน placeholder เป็น "ป้อนข้อมูล/concept...". `generateComicPanels` คืน `{concept, sharedSetting, panels}`
