 ...
 *
 * ใช้งาน:
 *   node manao.js                          รัน pipeline เต็ม (ไม่รวม post)
 *   node manao.js --date 2026-06-01        ประมวลข่าวของวันนั้น
 *   node manao.js --post --schedule        pipeline + โพสต์ FB+IG แบบ schedule
 *   node manao.js --no-scrape              ข้าม Agent 1
 *   node manao.js --dry-run                แสดงแผน ไม่รันจริง
 *   node manao.js --force                  สร้างใหม่ทับของเดิม
 *   node manao.js --platform fb,ig         สร้างเฉพาะ platform ที่ระบุ
 */
...
console.log('\n' + '═'.repeat(58));
console.log('🍋  มะนาว — AI News Orchestrator');
...
if (!DO_POST) {
  ...
  console.log('  หรือรันทุกอย่าง: node manao.js --post --schedule');
}
console.log('═'.repeat(58) + '\n');
