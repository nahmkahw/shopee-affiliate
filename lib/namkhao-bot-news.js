...
function _spawnPost(aiNewsDir, slug, platform, env, extraArgs) {
  return new Promise((resolve) => {
    // aiNewsDir = pipelineRoot (ที่เก็บข้อมูล news/)
    // post.js อาจอยู่ใน aiNewsDir หรือต้อง fallback ไปที่ manao/pipeline
    const postScript = fs.existsSync(path.join(aiNewsDir, 'post.js'))
      ? path.join(aiNewsDir, 'post.js')
      : path.join(__dirname, '..', 'agents', 'manao', 'pipeline', 'post.js');
    const proc = spawn(process.execPath, [postScript, slug, ...extraArgs, '--platform', platform], {
      cwd:   path.dirname(postScript),
      stdio: ['ignore', 'pipe', 'pipe'],
      env:   { ...process.env, ...env, PIPELINE_ROOT: aiNewsDir },
    });
...
function schedulePost(aiNewsDir, slug, platform = 'fb', env = {}) {
  return _spawnPost(aiNewsDir, slug, platform, env, ['--schedule']);
}

// โพสต์ทันทีไม่ schedule — ใช้โดย pipeline ที่ไม่มี auto-scheduler ประจำวัน (เช่น มะพร้าว, on-demand)
function postNow(aiNewsDir, slug, platform = 'fb', env = {}) {
  return _spawnPost(aiNewsDir, slug, platform, env, []);
}
...
async function handleNewsCallback(cbData, cbq, cbChat, { tgRequest, sendMsg, loadQueueFn, newsDir, aiNewsDir, env, log }) {
  ...
  if (cbData.startsWith('approve:') || cbData.startsWith('cancel:') || cbData.startsWith('regen:')) {
    ...
    const resolvedDir = (typeof entry === 'object' && entry.pipelineRoot) ? entry.pipelineRoot : aiNewsDir;
    const mode        = (typeof entry === 'object' && entry.mode === 'immediate') ? 'immediate' : 'schedule';
    if (log) log(`[queue] action=${action} shortId=${shortId} slug=${slug} platform=${platform} dir=${resolvedDir} mode=${mode}`);

    if (action === 'approve') {
      const waitMsg = mode === 'immediate' ? 'กำลังโพสต์...' : 'กำลัง schedule...';
      await tgRequest('answerCallbackQuery', { callback_query_id: cbq.id, text: `⏳ ${waitMsg}` });
      await sendMsg(cbChat, `⏳ <b>${waitMsg}</b>\n<code>${slug}</code>`);
      if (log) log(`⏳ ${waitMsg} (queue): ${slug} [${platform}]`);

      const { code, output } = mode === 'immediate'
        ? await postNow(resolvedDir, slug, platform, env)
        : await schedulePost(resolvedDir, slug, platform, env);
      if (code === 0) {
        const timeMatch = output.match(/กำหนดโพสต์:\s*(.+)/);
        const timeStr   = timeMatch ? timeMatch[1].trim() : '';
        const okLabel   = mode === 'immediate' ? 'โพสต์สำเร็จ!' : 'Schedule สำเร็จ!';
        if (log) log(`✅ ${mode === 'immediate' ? 'posted' : 'scheduled'}: ${slug}`);
        await sendMsg(cbChat, `✅ <b>${okLabel}</b>\n<code>${slug}</code>\n` + (timeStr ? `⏰ <b>กำหนดโพสต์:</b> ${timeStr}` : ''));
      } else {
        const failLabel = mode === 'immediate' ? 'โพสต์ล้มเหลว' : 'Schedule ล้มเหลว';
        if (log) log(`❌ ${failLabel}: ${slug} (code=${code})`);
        const errSnip = output.slice(-200).replace(/</g, '&lt;').replace(/>/g, '&gt;');
        await sendMsg(cbChat, `❌ <b>${failLabel}</b> (code=${code})\n<code>${slug}</code>\n` + (errSnip ? `<pre>${errSnip}</pre>` : ''));
      }
    } else if (action === 'cancel') {
      setNewsStatus(path.join(resolvedDir, 'news'), slug, 'skipped');
  ...
  // ── ส่งข่าวให้มะพร้าวสร้างการ์ตูน/วิดีโอ ─────────────────────────────────────
  if (cbData.startsWith('comic:') || cbData.startsWith('cvideo:')) {
    const colonIdx  = cbData.indexOf(':');
    const cbAction  = cbData.substring(0, colonIdx);
    const shortId   = cbData.substring(colonIdx + 1);
    const queue     = loadQueueFn();
    const entry     = queue[shortId];
    ...
    const slug         = typeof entry === 'string' ? entry : entry.slug;
    const prootEntry   = (typeof entry === 'object' && entry.pipelineRoot) ? entry.pipelineRoot : aiNewsDir;
    const source       = prootEntry.includes('makrut') ? 'makrut' : 'manao';
    const appRoot      = path.resolve(prootEntry, '..', '..', '..');
    const runScript    = path.join(appRoot, 'agents', 'maprao', 'run.js');
    const mode         = cbAction === 'cvideo' ? 'video' : 'comic';
    const newId        = Date.now().toString();
    const modeLabel    = mode === 'video' ? 'การ์ตูน + วิดีโอ' : 'การ์ตูน 4 ช่อง';

    await tgRequest('answerCallbackQuery', { callback_query_id: cbq.id, text: `⏳ ส่งให้มะพร้าวแล้ว` });
    await sendMsg(cbChat, `🥥 <b>ส่งข่าวให้มะพร้าวแล้ว</b> (${modeLabel})\n<code>${slug}</code>\nดูความคืบหน้าที่ /dashboard/maprao`);
    if (log) log(`[maprao] spawn comic-from-news source=${source} slug=${slug} id=${newId} mode=${mode}`);

    const proc = spawn(process.execPath, [
      runScript,
      '--action', 'comic-from-news',
      '--source', source,
      '--slug',   slug,
      '--id',     newId,
      ...(mode === 'video' ? ['--mode', 'video'] : []),
    ], { cwd: appRoot, stdio: 'ignore', detached: true, env: { ...process.env } });
    proc.unref();
    return true;
  }
...
module.exports = { loadQueue, resolveSlug, setNewsStatus, schedulePost, postNow, handleNewsCallback };
