 ...
 *
 * branch guard: DEPLOY_PATH คือ repo จริงที่คนอาจ checkout ค้างไว้ที่ feature branch
 *   `git pull --ff-only origin master` บน branch อื่น = ดึง master มาทับ branch นั้น (พัง)
 *   → หยุดทันที ให้คนไป checkout เองก่อน (ปลอดภัยกว่า auto-checkout ที่อาจทับงานค้าง)
 *
 * @returns {{proceed:boolean, reason?:string, conflicts?:string[], npmCi:boolean, behind:number}}
 */
function evaluate({ localModified = [], upstreamChanged = [], behind = 0, currentBranch, targetBranch } = {}) {
  if (currentBranch && targetBranch && currentBranch !== targetBranch) {
    return { proceed: false, reason: 'wrong-branch', currentBranch, targetBranch, npmCi: false, behind };
  }
  if (behind === 0) {
