...
function preCheck() {
  ...
  return { ...evaluate({ localModified, upstreamChanged, behind }), upstreamChanged };
}
...
async function main() {
  ...
  const pre = preCheck();
  if (!pre.proceed) {
    const msg = pre.reason === 'up-to-date'
      ? `ℹ️ Deploy ข้าม — ${BRANCH} ไม่มีอะไรใหม่ (HEAD ${before.slice(0, 7)})`
      : `❌ Deploy หยุด — ไฟล์แก้ค้างชนกับ upstream:\n${pre.conflicts.map(c => `• ${c}`).join('\n')}\ncommit/stash ก่อนแล้วกดใหม่`;
    log(msg);
