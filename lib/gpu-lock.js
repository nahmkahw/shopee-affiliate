...
// ── waiters registry (ไฟล์ต่อ pid — ไม่มี shared-write race) สำหรับแสดงผล queue ──
const WAITER_PREFIX = LOCK_FILE.replace(/\.lock$/, '') + '.waiter.';
const waiterFile = pid => `${WAITER_PREFIX}${pid}.json`;
function registerWaiter(label) {
  try { fs.writeFileSync(waiterFile(process.pid), JSON.stringify({ agent: label, since: Date.now() })); } catch {}
}
function deregisterWaiter() { try { fs.unlinkSync(waiterFile(process.pid)); } catch {} }

async function acquire(label) {
  if (tryAcquireOnce(label)) return;                  // ว่าง → ได้เลย ไม่ต้องลงคิว
  registerWaiter(label);
  process.on('exit', deregisterWaiter);
  try {
    ...
    deregisterWaiter();
    process.removeListener('exit', deregisterWaiter);
  }
}

// อ่านสถานะ queue สำหรับ dashboard: { holder, waiters[] } + prune PID ที่ตาย
function readQueueStatus() {
  const l = readLock();
  const holder = (l && l.pid && isAlive(l.pid)) ? { agent: l.agent, pid: l.pid, since: l.since } : null;
  const waiters = [];
  const dir = path.dirname(WAITER_PREFIX);
  const base = path.basename(WAITER_PREFIX);
  try {
    for (const f of fs.readdirSync(dir)) {
      if (!f.startsWith(base) || !f.endsWith('.json')) continue;
      const pid  = parseInt(f.slice(base.length), 10);
      const full = path.join(dir, f);
      if (!isAlive(pid)) { try { fs.unlinkSync(full); } catch {} continue; }   // prune dead
      try { const w = JSON.parse(fs.readFileSync(full, 'utf8')); waiters.push({ agent: w.agent, pid, since: w.since }); } catch {}
    }
  } catch {}
...
module.exports = { withGpuLock, acquire, release, readLock, readQueueStatus };
