...
function findThaiFont() {
  ...
  _cachedFont = candidates.find(f => fs.existsSync(f)) || 'C:\\Windows\\Fonts\\Arial.ttf';
  return _cachedFont;
}
...
function createSceneClip(topImgPath, bgImgPath, thumbImgPath, qrCodePath, affiliateUrl, audioPath, audioDuration, voiceoverText, outputPath) {
  ...
  const filterLines = [
    ...
    `[3:v]${qrFilter}[qr]`,
    `[top][thumb]overlay=x=W-w-20:y=H-h-20[topth]`,
    `[topth][qr]overlay=x=20:y=H-h-20[topfull]`,
    `[topfull][${bgCurrent}]vstack=inputs=2[out]`,
  ];
