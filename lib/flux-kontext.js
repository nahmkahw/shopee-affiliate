 ...
 * Shared by มะปราง (anime video/comic) and มะพร้าว (B&W manga comic) — Gate 2.
 *
 * ปัญหา: T2V/T2I วาดตัวละครใหม่ทุก scene → หน้าตาเปลี่ยน
 * วิธีแก้: ใช้ ref image เป็น anchor → Flux Kontext วางตัวละครเดิม (หน้า/ผม/ชุด คงที่)
...
const { submitImageWorkflow, uploadImageToComfy } = require('./comfy-client-core');
 ...
 * @param {string} [styleSuffix] ปรับ art style ท้าย instruction (default: anime style สีสัน)
 */
function buildInstruction(sceneSetting, names = [], styleSuffix) {
  const style = styleSuffix || 'Anime style, cinematic lighting, full body visible, detailed background.';
  if (names.length > 1) {
    const order = names.map((n, i) => `${i === 0 ? 'on the left' : i === names.length - 1 ? 'on the right' : 'in the middle'} ${n}`).join(', ');
    return `Place these ${names.length} characters together into the scene, keeping EACH character ` +
      ...
      `They interact naturally in the same scene. ${style}`;
  }
  return 'Place this exact same character, keeping the identical face, hairstyle, ' +
...
function buildKontextWorkflow(refFilenames, instruction, seed) {
  ...
  Object.assign(wf, {
    ...
    '13': { class_type: 'SaveImage',      inputs: { images: ['12', 0], filename_prefix: 'flux_kontext' } },
  });
  return wf;
}
 ...
 * @param {object} [opts]         { seed, names, styleSuffix, lockLabel }
...
async function generateSceneStill(cfg, refImagePaths, sceneSetting, outPath, opts = {}) {
  ...
  const instruction = buildInstruction(sceneSetting, opts.names || [], opts.styleSuffix);
  console.log(`  🎨 Flux Kontext (${refs.length} char): "${sceneSetting.substring(0, 45)}..."`);
  const { outputPath, bytes } = await submitImageWorkflow(
    cfg, buildKontextWorkflow(filenames, instruction, opts.seed), '13', outPath, KONTEXT_TIMEOUT, opts.lockLabel || 'flux-kontext');
