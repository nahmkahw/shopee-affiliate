...
/**
 * textMessage — สร้าง message object (แนบ Quick Reply chips ได้)
 * @param {string} text
 * @param {Array<{label,text}>} [quickReply]  ปุ่ม message-action (แตะแล้วส่งข้อความกลับ)
 */
function textMessage(text, quickReply) {
  const msg = { type: 'text', text: String(text).slice(0, 4900) };
  if (Array.isArray(quickReply) && quickReply.length) {
    msg.quickReply = { items: quickReply.slice(0, 13).map(q => ({
      type: 'action',
      action: { type: 'message', label: String(q.label).slice(0, 20), text: q.text },
    })) };
  }
  return msg;
}
 ...
 * @param {string} text
 * @param {Array<{label,text}>} [quickReply]
 */
function pushMessage(to, text, token, quickReply) {
  return _request({
    method: 'POST', path: '/v2/bot/message/push', token,
    jsonBody: { to, messages: [textMessage(text, quickReply)] },
  });
}
 ...
 */
function replyMessage(replyToken, text, token, quickReply) {
  return _request({
    method: 'POST', path: '/v2/bot/message/reply', token,
    jsonBody: { replyToken, messages: [textMessage(text, quickReply)] },
  });
}
