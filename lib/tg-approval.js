...
/**
 * ส่งวิดีโอไป Telegram (ไม่มีปุ่ม — ใช้แจ้งเตือนหลังสร้างวิดีโอเสร็จ)
 * @param {string} videoPath  path ไฟล์ .mp4 บนเครื่อง
 * @param {string} caption    HTML caption (≤1024 chars)
 * @returns {Promise<object>} Telegram API response
 */
function sendVideoToTelegram(videoPath, caption) {
  const token  = process.env.NAMKHAO_TELEGRAM_BOT_TOKEN || process.env.TELEGRAM_BOT_TOKEN;
  const chatId = process.env.TELEGRAM_CHAT_ID;
  if (!token || !chatId) return Promise.resolve({ ok: false, description: 'no token/chatId' });

  const boundary = '----TGVid' + Math.random().toString(36).substring(2);
  const vidBuf   = fs.readFileSync(videoPath);
  const fileName = path.basename(videoPath);
  const fieldStr = `--${boundary}\r\nContent-Disposition: form-data; name="chat_id"\r\n\r\n${chatId}\r\n` +
    `--${boundary}\r\nContent-Disposition: form-data; name="caption"\r\n\r\n${(caption || '').substring(0, 1024)}\r\n` +
    `--${boundary}\r\nContent-Disposition: form-data; name="parse_mode"\r\n\r\nHTML\r\n`;
  const vidHeader = `--${boundary}\r\nContent-Disposition: form-data; name="video"; filename="${fileName}"\r\nContent-Type: video/mp4\r\n\r\n`;
  const footer    = `\r\n--${boundary}--\r\n`;
  const headerBuf = Buffer.from(fieldStr + vidHeader, 'utf8');
  const footerBuf = Buffer.from(footer, 'utf8');
  const totalLen  = headerBuf.length + vidBuf.length + footerBuf.length;
  ...
  return new Promise(resolve => {
    ...
    req.write(headerBuf); req.write(vidBuf); req.write(footerBuf); req.end();
  });
}
 ...
 */
async function sendApprovalNotification(slug, data, master, { pipelineRoot, newsDir, mode, emoji, kind, addMapraoButtons }) {
  ...
  if (addMapraoButtons) kbRows.push([
    { text: '🥥 การ์ตูน 4 ช่อง', callback_data: `comic:${shortId}`  },
    { text: '🎬 สร้างวิดีโอ',     callback_data: `cvideo:${shortId}` },
  ]);
  const replyMarkup = { inline_keyboard: kbRows };
...
module.exports = { sendApprovalNotification, sendVideoToTelegram };
