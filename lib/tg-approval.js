 ...
 * @param {string} slug
 * @param {object} data  — data.json ของข่าว
 * @param {string} master — บทความกลาง (ใช้เป็น preview)
 * @param {{ pipelineRoot: string, newsDir: string }} opts
 */
async function sendApprovalNotification(slug, data, master, { pipelineRoot, newsDir }) {
  const isMakrut = (pipelineRoot || process.env.PIPELINE_ROOT || '').includes('makrut');
  const token  = isMakrut
  ...
  queue[shortId]  = { slug, platform: 'fb', pipelineRoot };
  ...
  if (hasLocalImg)  return _sendPhotoMultipart(token, chatId, imagePath, text, replyMarkup);
  if (ogImageUrl)   return _sendPhotoUrl(token, chatId, ogImageUrl, text, replyMarkup);
