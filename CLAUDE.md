...
├── agent-hub.js           ← Multi-agent control hub (port 3002) รวม mali + manao
...
`agent-hub.js` เป็น HTTP server ควบคุม sub-agents ผ่าน web UI:
...
- FB Reels ใช้ Page Access Token (แลกจาก User Token ใน `agent-hub.js` อัตโนมัติ)
...
node agent-hub.js                 # เปิด Agent Hub ที่ http://localhost:3002
...
| FB Reels `(#200) no permission` | Reels ต้องใช้ Page Token (ไม่ใช่ User Token) — `agent-hub.js` exchange ให้อัตโนมัติแล้ว |
