...
- **Dashboard live update:** Gallery card ที่ `status=producing` ถูก poll `GET /api/maprao/status/{id}` ทุก 5s → replace เฉพาะ card DOM นั้น (ไม่ reload ทั้งหน้า); polling หยุดอัตโนมัติเมื่อ status พ้น producing
