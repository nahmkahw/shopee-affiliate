...
- **OCR (`lib/slip-ocr.js`, Gate 2 pluggable):** Ollama vision (`MAYOM_OCR_MODEL=qwen2.5vl:latest` default, เผื่อสลับ EasySlip) คืน `{is_slip, amount, slip_datetime, bank_from/to, ref_no}` — `is_slip=false` → เงียบ ไม่บันทึก; normalize ปี พ.ศ.→ค.ศ. ใน `slip_datetime`; ย่อรูป > `MAYOM_OCR_MAXW`(1600) ก่อน OCR (ตั้งสูงเพราะย่อต่ำ→ข้อความไทยเพี้ยน; latency หลัก = cold-load โมเดล ไม่ใช่ขนาดรูป — warm ~2.5s/ใบ)
